...
CUDA_VISIBLE_DEVICES=7 python OpenNMT-py/translate.py \
				 -gpu 0 -replace_unk -alpha 0.6 -beta 0.0 \ 
				-beam_size 5 -length_penalty wu -coverage_penalty wu \
				-model $MODEL -src data/news-commentary-v11.de-en.en.sp \
				—output data/prediction.txt -verbose
